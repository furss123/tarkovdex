...
function coveredAreas(item: ArmorItem): string[] {
  return areasOf([
    ...
    ...item.slots.flatMap((slot) => slot.normalizedZones),
  ]);
}

/** Practical protection level: the armor's own class, else its best soft layer, else its best plate. */
function effectiveClass(item: ArmorItem): number | null {
  const values = [
    item.armorClass,
    ...item.softArmor.map((layer) => layer.armorClass),
    ...item.slots.flatMap((slot) => slot.allowedPlates.map((plate) => plate.armorClass)),
...
export function ArmorExplorer({ regular, pve }: { regular: CombatDataset; pve: CombatDataset }) {
  ...
  const armor = useMemo(
    () => source
      .map((item) => ({ item, cls: effectiveClass(item), areas: coveredAreas(item) }))
  ...
  return (
    <div>
      <div className="grid gap-3 rounded-lg border border-border bg-surface/30 p-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <label className="flex min-h-touch items-center gap-2 self-end text-xs text-muted"><input type="checkbox" checked={replaceable} onChange={(event) => setReplaceable(event.target.checked)} className="size-4 accent-accent" />{t('replaceable')}</label>
      </div>
      ...
      <div className="mt-5 space-y-3">
        {visible.slice(0, limit).map(({ item, cls, areas }) => <ArmorCard key={item.id} item={item} cls={cls} areas={areas} />)}
      </div>
    ...
    <p className="flex flex-wrap gap-1">
      {AREAS.map((value) => {
        ...
        return (
          <span
            key={value}
            className={`inline-flex items-center rounded border px-1.5 py-0.5 text-[12px] leading-4 ${on ? 'border-accent/50 bg-accent/10 text-accent' : 'border-border/60 text-muted/45'}`}
          >
            {t(`areas.${value}`)}
          </span>
        );
...
function ArmorCard({ item, cls, areas }: { item: ArmorItem; cls: number | null; areas: string[] }) {
  const t = useTranslations('armor');
  ...
  return (
    <details className="group rounded-lg border border-border bg-surface/20 open:border-accent/30">
      <summary className="grid min-h-touch cursor-pointer list-none grid-cols-[auto_auto_1fr_auto] items-center gap-x-2.5 gap-y-2 p-3 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/50 sm:gap-x-3 sm:p-4">
        <span className="flex size-[42px] flex-col items-center justify-center rounded-md border border-border bg-bg leading-none sm:size-[56px]">
          <span className="text-[11px] text-muted">{t('classShort')}</span>
          <span className="mt-0.5 text-lg tabular-nums text-accent">{cls ?? '—'}</span>
        </span>
        {item.iconLink ? <>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={item.iconLink} alt="" width={56} height={56} loading="lazy" className="size-[42px] object-contain sm:size-[56px]" />
        </> : <Shield className="size-[42px] p-2 text-muted sm:size-[56px]" />}
        <div className="min-w-0">
          <h2 className="break-words text-sm font-medium text-fg">{item.name}</h2>
          <p className="mt-1 text-xs text-muted">
            {item.weight === null ? '—' : `${item.weight} kg`} · {t('speed')} {pct(item.speedPenalty)} · {t('ergo')} {pct(item.ergoPenalty)}
      ...
      <div className="border-t border-border p-4">
        <dl className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5">
          ...
          <Stat label={t('material')} value={localizeMaterial(item.material, locale) ?? '—'} />
              ...
              <div key={index} className="rounded-md border border-border p-3">
                ...
                <ul className="mt-2 grid gap-x-4 gap-y-1.5 md:grid-cols-2 xl:grid-cols-3">
                  {group.plates.map((plate) => (
                    <li key={plate.id} className="flex items-baseline gap-2 text-xs">
                      <span className="w-6 shrink-0 tabular-nums text-accent">{plate.armorClass ?? '—'}</span>
                      <span className="min-w-0 flex-1 break-words text-fg">{plate.name}</span>
                      <span className="shrink-0 tabular-nums text-muted">{plate.weight === null ? '—' : `${plate.weight}kg`} · {plate.durability ?? '—'}</span>
                    </li>
                  ))}
                </ul>
              </div>
